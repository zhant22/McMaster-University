...
def NNClassifier(X, t, hidden_layer_sizes, initialize, epochs, learning_rate):
    ...
    return output_best, np.min(loss), w_1_best , w_2_best , w_2_best
...
for n1 in n_values:
    for n2 in range(1, 8 - n1 + 1):
        ...
        for i in range(3):
            o, l ,_,_,_= NNClassifier(X_train, t_train, hidden_layer_sizes=(n1, n2), initialize=random_integers, epochs=10, learning_rate=0.005)
            ...
            for epoch in range(50):
                o_valid, _ ,w1,w2,w3= NNClassifier(X_valid_sub, t_valid_sub, hidden_layer_sizes=(n1, n2), initialize=random_integers, epochs=10, learning_rate=0.005)
...
print("Best Weight 3:", best_w2)

# training the model with the n1 and n2 we choose 
...
for i in range(100):
    # Training the final model with the best n1 and n2
    best_output, l1 ,w1,w2,w3= NNClassifier(X_train, t_train, hidden_layer_sizes=(best_n1, best_n2), initialize=random_integers, epochs=20, learning_rate=0.05)

    # Calculate misclassification error on the training set
    if l1>0.1:
        loss_Training.append(l1)
        loss2_Training.append(misclassification_error(best_output,t_train))

    entropy_loss_Training.append(np.mean(loss_Training))
    mis_rate_Training.append(np.mean(loss2_Training))

    train_misclassification_error = misclassification_error(best_output, t_train)


#Calculate misclassification error on the validation set
for i in range(100):
    valid_output, l2 ,w1,w2,w3= NNClassifier(X_valid, t_valid, hidden_layer_sizes=(best_n1, best_n2), initialize=random_integers, epochs=20, learning_rate=0.005)
    
    if l2>0.1:
        loss_valid.append(l2)
        loss2_valid.append(misclassification_error(best_output,t_train))

    entropy_loss_valid.append(np.mean(loss_valid))
    mis_rate_valid.append(np.mean(loss2_valid))
...
for i in range(100):
    test_output, l3 ,w1,w2,w3= NNClassifier(X_test, t_test, hidden_layer_sizes=(best_n1, best_n2), initialize=random_integers, epochs=20, learning_rate=0.005)

    if l3>0.1:
        loss_test.append(l3)
        loss2_test.append(misclassification_error(best_output,t_train))

    entropy_loss_test.append(np.mean(loss_test))
    mis_rate_test.append(np.mean(loss2_test))
